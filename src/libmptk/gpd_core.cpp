...
unsigned short int GPD_Core_c::step() {
  ...
  //unsigned long int gradLen;
  MP_Signal_c *sigGrad;
  GP_Pos_Range_Sub_Book_Iterator_c iter;
  MP_Real_t alpha, enCorr, tmp;
  ...
  for (iter = touchBook->begin(); iter !=touchBook->end(); ++iter){
      //iter->info( stderr );
	  for (c = 0; c<iter->numChans; c++){
		  if (gradSupport.len < iter->support[c].pos+dict->block[iter->blockIdx]->filterLen-1)
			  gradSupport.len = iter->support[c].pos+dict->block[iter->blockIdx]->filterLen-1;
		  if (gradSupport.pos > iter->support[c].pos)
			  gradSupport.pos = iter->support[c].pos;
	  }
      size++;
  }  
  
  gradSupport.len = gradSupport.len-gradSupport.pos+1;
    
  sigGrad = MP_Signal_c::init(residual, gradSupport);
  sigGrad->fill_zero();
  ...
  for (c=0; c<dict->signal->numChans; c++){
	  ...
	  for (iter = touchBook->begin(); iter !=touchBook->end(); ++iter){
		/* temporarilly set the amplitude to the correlation */

		tmp = iter->amp[c];
		iter->amp[c] = iter->corr[c];

		/* temporarilly offset the position */
		iter->support[c].pos = iter->support[c].pos - gradSupport.pos;
	//    mp_progress_msg( "Computing gradient", "relative pos = %lu\n", iter->atom->support[0].pos);
	//    mp_progress_msg( "Computing gradient", "registered amplitude = %lf\n", tmp);
	//    mp_progress_msg( "Computing gradient", "registered correlation = %lf\n", iter->corr[0]);
		iter->substract_add(NULL, sigGrad);
	//    mp_progress_msg( "\t", "Block = %u\tPosition = %u\tFreq = %u\tCorr = %lf\t,sigGrad = %p\n",
	//                     iter->blockIdx,
	//                     iter->pos.pos[0],
	//                     iter->param.value,
	//                     iter->atom->amp[0],
	//                     sigGrad);
		/* restore the original atom parameters */
		iter->amp[c] = tmp;
	  ...
	  alpha = .5*enCorr/sigGrad->compute_energy();
	//  mp_progress_msg( "Computing gradient", "Alpha = %lf\n", alpha);
	//  mp_progress_msg( "Blabla", "enCorr = %lf\n", enCorr);
	//  mp_progress_msg( "Blabla", "grad.energy = %lf\n", sigGrad->compute_energy());
	//
	  for (iter = touchBook->begin(); iter !=touchBook->end(); ++iter){
			tmp = iter->amp[c];
			iter->amp[c] = (iter->corr[c])*alpha;
			//        iter->info(stderr);
	        //mp_progress_msg( "Updating residual", "correlation = %lf\n", iter->corr[c]);
			iter->substract_add(residual, NULL);
			iter->amp[c] += tmp;
	  }
