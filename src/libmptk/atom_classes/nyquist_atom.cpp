...
/*************************************************/
/*                                               */
/* nyquist_atom.cpp: methods for nyquist atoms */
/*                                               */
/*************************************************/
...
MP_Nyquist_Atom_c* MP_Nyquist_Atom_c::init( const MP_Chan_t setNumChans ) {

  const char* func = "MP_Nyquist_Atom_c::init(numChans)";
  MP_Nyquist_Atom_c* newAtom = NULL;

  /* Instantiate and check */
  newAtom = new MP_Nyquist_Atom_c();
  if ( newAtom == NULL ) {
  ...
  if ( newAtom->global_alloc( setNumChans ) ) {
    mp_error_msg( func, "Failed to allocate some vectors in the new Nyquist atom.\n" );
    delete( newAtom );
    return( NULL );
  }
...
MP_Nyquist_Atom_c* MP_Nyquist_Atom_c::init( FILE *fid, const char mode ) {

  const char* func = "MP_Nyquist_Atom_c::init(fid,mode)";
  MP_Nyquist_Atom_c* newAtom = NULL;
  ...
  if ( newAtom->read( fid, mode ) ) {
    mp_error_msg( func, "Failed read the new Nyquist atom.\n" );
    delete( newAtom );
    return( NULL );
  }
...
/********************/
/* Void constructor */
MP_Nyquist_Atom_c::MP_Nyquist_Atom_c( void )
  :MP_Atom_c() {
}
...
/************************/
/* Global allocations  */
int MP_Nyquist_Atom_c::global_alloc( const MP_Chan_t setNumChans ) {

  const char* func = "MP_Nyquist_Atom_c::internal_alloc(numChans)";

  /* Go up one level */
  if ( MP_Atom_c::global_alloc( setNumChans ) ) {
    mp_error_msg( func, "Allocation of Nyquist atom failed at the generic atom level.\n" );
    return( 1 );
  }
...
/********************/
/* File reader      */
int MP_Nyquist_Atom_c::read( FILE *fid, const char mode ) {
...
/**************/
/* Destructor */
MP_Nyquist_Atom_c::~MP_Nyquist_Atom_c() {
}
...
int MP_Nyquist_Atom_c::write( FILE *fid, const char mode ) {
...
/*************/
/* Type name */
char * MP_Nyquist_Atom_c::type_name(void) {
  return ("nyquist");
}

/**********************/
/* Readable text dump */
int MP_Nyquist_Atom_c::info( FILE *fid ) {
...
/**********************/
/* Readable text dump */
int MP_Nyquist_Atom_c::info() {

  unsigned int i = 0;
  int nChar = 0;

  nChar += mp_info_msg( "NYQUIST ATOM", "[%d] channel(s)\n", numChans );
...
/********************/
/* Waveform builder */
void MP_Nyquist_Atom_c::build_waveform( MP_Sample_t *outBuffer ) {
...
int MP_Nyquist_Atom_c::add_to_tfmap( MP_TF_Map_c *tfmap, const char /* tfmapType */ ) {
...
int MP_Nyquist_Atom_c::has_field( int field ) {
...
MP_Real_t MP_Nyquist_Atom_c::get_field( int field , MP_Chan_t chanIdx ) {
