...
/************************/
/* Factory function     */
MP_Mdct_Atom_c * MP_Mdct_Atom_c::init( const MP_Chan_t setNumChans,
                                       const unsigned char setWindowType,
                                       const double setWindowOption ) {
  
  const char* func = "MP_Mdct_Atom_c::MP_Mdct_Atom_c(...)";
  
  MP_Mdct_Atom_c* newAtom = NULL;

  /* Instantiate and check */
  newAtom = new MP_Mdct_Atom_c();
  if ( newAtom == NULL ) {
    mp_error_msg( func, "Failed to create a new MDCT atom. Returning a NULL.\n" );
    return( NULL );
  }
  ...
  if ( newAtom->global_alloc( setNumChans ) ) {
    mp_error_msg( func, "Failed to allocate some vectors in the new MDCT atom. Returning a NULL atom.\n" );
    delete( newAtom );
    return( NULL );
  }
...
MP_Mdct_Atom_c* MP_Mdct_Atom_c::init( FILE *fid, const char mode ) {
  
  const char* func = "MP_Mdct_Atom_c::init(fid,mode)";
...
/********************/
/* Void constructor */
MP_Mdct_Atom_c::MP_Mdct_Atom_c( void )
  :MP_Atom_c() {
...
/************************/
/* Local allocations    */
int MP_Mdct_Atom_c::local_alloc( const MP_Chan_t /* setNumChans */ ) {

  // const char* func = "MP_Mdct_Atom_c::local_alloc(numChans)";
...
/************************/
/* Global allocations   */
int MP_Mdct_Atom_c::global_alloc( const MP_Chan_t setNumChans ) {

  const char* func = "MP_Mdct_Atom_c::global_alloc(numChans)";

  /* Go up one level */
  if ( MP_Atom_c::global_alloc( setNumChans ) ) {
    mp_error_msg( func, "Allocation of MDCT atom failed at the generic atom level.\n" );
    return( 1 );
  }
...
/***************/
/* File reader */
int MP_Mdct_Atom_c::read( FILE *fid, const char mode ) {

  const char* func = "MP_Mdct_Atom_c::MP_Mdct_Atom_c(fid,mode)";
...
int MP_Mdct_Atom_c::write( FILE *fid, const char mode ) {
...
/*************/
/* Type name */
char * MP_Mdct_Atom_c::type_name(void) {
  return ("mdct");
}

/**********************/
/* Readable text dump */
int MP_Mdct_Atom_c::info( FILE *fid ) {
...
/********************/
/* Waveform builder */
void MP_Mdct_Atom_c::build_waveform( MP_Sample_t *outBuffer ) {
...
int MP_Mdct_Atom_c::add_to_tfmap( MP_TF_Map_c *tfmap, const char tfmapType ) {
  ...
  for (chanIdx = 0; chanIdx < numChans; chanIdx++) {
    ...
    gatom->phase[chanIdx] = freq * ( 1/2 + support[chanIdx].len/4 );
  }
  // Plotting
...
/* Sorting function which characterizes various properties of the atom,
   along one channel */
int MP_Mdct_Atom_c::has_field( int field ) {
...
MP_Real_t MP_Mdct_Atom_c::get_field( int field, MP_Chan_t chanIdx ) {
  MP_Real_t x;
