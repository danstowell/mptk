...
/***********************************************/
/* Factory function  for a strict mdct block   */
MP_Mdct_Block_c* MP_Mdct_Block_c::init( MP_Signal_c *setSignal,
					  const unsigned long int setFilterLen,
					  const unsigned char setWindowType,
					  const double setWindowOption,
                                          const unsigned long int setBlockOffset ) {

  const char* func = "MP_Mdct_Block_c::init()";

  /* Parameters for a strict MDCT */
  ...
  /* Call the factory function of a generalized mdct */
  MP_Mdct_Block_c *newBlock = init(setSignal,setFilterLen,setFilterShift,setFftSize,setWindowType,setWindowOption,setBlockOffset);

  return( newBlock );
}

/****************************************************/
/* Factory function  for a generalized mdct block   */
MP_Mdct_Block_c* MP_Mdct_Block_c::init( MP_Signal_c *setSignal,
					  const unsigned long int setFilterLen,
  ...
  const char* func = "MP_Mdct_Block_c::init()";
  MP_Mdct_Block_c *newBlock = NULL;

  /* Instantiate and check */
  newBlock = new MP_Mdct_Block_c();
  if ( newBlock == NULL ) {
    mp_error_msg( func, "Failed to create a new Mdct block.\n" );
    return( NULL );
  }
  ...
  if ( newBlock->init_parameters( setFilterLen, setFilterShift, setFftSize,
    ...
    mp_error_msg( func, "Failed to initialize some block parameters in the new Mdct block.\n" );
    delete( newBlock );
    return( NULL );
  }
...
int MP_Mdct_Block_c::init_parameters( const unsigned long int setFilterLen,
				       const unsigned long int setFilterShift,
				       const unsigned long int setFftSize,
				       const unsigned char setWindowType,
				       const double setWindowOption ,
...
const char* func = "MP_Mdct_Block_c::init_parameters()";
...
int MP_Mdct_Block_c::plug_signal( MP_Signal_c *setSignal ) {
...
void MP_Mdct_Block_c::nullify_signal( void ) {
...
/********************/
/* NULL constructor */
MP_Mdct_Block_c::MP_Mdct_Block_c( void )
...
/**************/
/* Destructor */
MP_Mdct_Block_c::~MP_Mdct_Block_c() {
...
/********/
/* Type */
char * MP_Mdct_Block_c::type_name() {
  return ("mdct");
}

/**********************/
/* Readable text dump */
int MP_Mdct_Block_c::info( FILE *fid ) {
...
void MP_Mdct_Block_c::update_frame(unsigned long int frameIdx, 
				    MP_Real_t *maxCorr, 
...
unsigned int MP_Mdct_Block_c::create_atom( MP_Atom_c **atom,
					    const unsigned long int frameIdx,
					    const unsigned long int freqIdx ) {

  const char* func = "MP_Mdct_Block_c::create_atom(...)";
  MP_Mdct_Atom_c *matom = NULL;
  ...
  /* Allocate the atom */
  *atom = NULL;
  if ( (matom = MP_Mdct_Atom_c::init( s->numChans, fft->windowType, fft->windowOption )) == NULL ) {
    mp_error_msg( func, "Can't create a new Mdct atom in create_atom()."
...
int MP_Mdct_Block_c::alloc_energy( MP_Real_t **atomEnergy ) {
 ...
 */
int MP_Mdct_Block_c::fill_energy( MP_Real_t *atomEnergy ) {
