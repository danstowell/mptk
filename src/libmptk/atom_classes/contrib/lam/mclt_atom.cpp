...
/****************************************************************/
/*                                                              */
/* mclt_atom.cpp: methods for mclt atoms			*/
/*                                                              */
/****************************************************************/
...
/************************/
/* Factory function     */
MP_Mclt_Atom_c * MP_Mclt_Atom_c::init( const MP_Chan_t setNumChans,
                                       const unsigned char setWindowType,
                                       const double setWindowOption ) {
  
  const char* func = "MP_Mclt_Atom_c::MP_Mclt_Atom_c(...)";
  
  MP_Mclt_Atom_c* newAtom = NULL;

  /* Instantiate and check */
  newAtom = new MP_Mclt_Atom_c();
  if ( newAtom == NULL ) {
    mp_error_msg( func, "Failed to create a new MCLT atom. Returning a NULL.\n" );
    return( NULL );
  }
  ...
  if ( newAtom->global_alloc( setNumChans ) ) {
    mp_error_msg( func, "Failed to allocate some vectors in the new MCLT atom. Returning a NULL atom.\n" );
    delete( newAtom );
    return( NULL );
  }
...
MP_Mclt_Atom_c* MP_Mclt_Atom_c::init( FILE *fid, const char mode ) {
...
/********************/
/* Void constructor */
MP_Mclt_Atom_c::MP_Mclt_Atom_c( void )
  :MP_Atom_c() {
...
/************************/
/* Local allocations    */
int MP_Mclt_Atom_c::local_alloc( const MP_Chan_t /* setNumChans */ ) {

  const char* func = "MP_Mclt_Atom_c::local_alloc(numChans)";

  /* phase */
  if ( (phase = (MP_Real_t*)malloc( numChans*sizeof(MP_Real_t)) ) == NULL ) {
    mp_error_msg( func, "Can't allocate the phase array for a new atom.\n" );
      return( 1 );
  }
  /* Initialize */
  if ( phase!=NULL ) {
...
/************************/
/* Global allocations   */
int MP_Mclt_Atom_c::global_alloc( const MP_Chan_t setNumChans ) {

  const char* func = "MP_Mclt_Atom_c::global_alloc(numChans)";

  /* Go up one level */
  if ( MP_Atom_c::global_alloc( setNumChans ) ) {
    mp_error_msg( func, "Allocation of MCLT atom failed at the generic atom level.\n" );
    return( 1 );
  }
...
/********************/
/* File reader      */
int MP_Mclt_Atom_c::read( FILE *fid, const char mode ) {
...
int MP_Mclt_Atom_c::write( FILE *fid, const char mode ) {
  ...
  /* Print the other Mclt-specific parameters */
  switch ( mode ) {
  ...
  case MP_TEXT:
    ...
    for (i = 0; i<numChans; i++) {
      nItem += fprintf( fid, "\t\t<McltPar chan=\"%u\">\n", i );
      nItem += fprintf( fid, "\t\t<par type=\"phase\">%g</par>\n", (double)phase[i] );
      nItem += fprintf( fid, "\t\t</McltPar>\n" );    
    }
    break;
...
/*************/
/* Type name */
char * MP_Mclt_Atom_c::type_name(void) {
  return ("mclt");
}

/**********************/
/* Readable text dump */
int MP_Mclt_Atom_c::info( FILE *fid ) {
...
/**********************/
/* Readable text dump */
int MP_Mclt_Atom_c::info() {

  unsigned int i = 0;
  int nChar = 0;

  nChar += mp_info_msg( "Mclt ATOM", "%s window (window opt=%g)\n", window_name(windowType), windowOption );
...
/********************/
/* Waveform builder */
void MP_Mclt_Atom_c::build_waveform( MP_Sample_t *outBuffer ) {
  ...
  for ( chanIdx = 0 , atomBuffer = outBuffer; 
    ...
    dPhase     = dFreq * ( 0.5 + len*0.25 ) + (double)( phase[chanIdx] );
    dAmp       = (double)(   amp[chanIdx] );

    /** Multiply by the desired modulation
     */
    for ( t = 0; t<len; t++ ) {
...
int MP_Mclt_Atom_c::add_to_tfmap( MP_TF_Map_c *tfmap, const char tfmapType ) {
  ...
  for (chanIdx = 0; chanIdx < numChans; chanIdx++) {
    ...
    gatom->phase[chanIdx] = phase[chanIdx];
  }
  // Plotting
