 ...
 */
class MP_Anywave_Hilbert_Block_c:public MP_Anywave_Block_c {
...
public:
  ...
  /** \brief An object for performing the inner products between a
   * signal and a hilbert transformed filter
   *
   */
  MP_Convolution_Direct_c* hilbertConvolution;
...
public:
  ...
  /** \brief Factory function for a Anywave Hilbert block
   *
   * a static initialization function that construct a new instance of
   * MP_Anywave_Hilbert_Block_c from a file containing the anywave table
   *
   * \param setSignal a pointer to the signal associated to the block
   *
   * \param setFilterShift the filter shift between two successive
   * atoms
   *
...
protected:
  ...
  MP_Anywave_Hilbert_Block_c( void );

  /** \brief Destructor
   **/
  virtual ~MP_Anywave_Hilbert_Block_c();
...
/** \brief Add a anywave hilbert block to a dictionary
 *
 * \param dict The dictionnary that will be modified
 *
 * \param filterShift shift, in samples, between two consecutive
 * frames.  Typically, use \a filterShift = \a filterLen / 2 to get 50
 * percent overlap between windows
 *
 * \param table A pointer to a MP_Anywave_Table_c object, containing the waveforms. 
 * The filterLen and numFilters parameters are given by table.
 *
 * \return one upon success, zero otherwise 
 *
 * \sa MP_Anywave_Block_c::MP_Anywave_Block_c()
 */
int add_anywave_hilbert_block( MP_Dict_c *dict,
			       const unsigned long int filterShift,
			       MP_Anywave_Table_c *table );


/** \brief Add a family of anywave hilbert blocks to a dictionary.
 *
 * The family is given by several anywave tables and several filter shifts. 
 * Each combination gives a block.
 *
 * \param dict The dictionnary that will be modified
 * \param numFilterShift the number of filter shifts
 * \param filterShiftPtr an array containing the different filter shifts of the blocks
 * \param numTables the number of anywave tables
 * \param tablePtr an array containing the different anywave tables of the blocks 
 * \return the number of added blocks
 *
 * \sa MP_Dict_c::add_anywave_block()
 */
