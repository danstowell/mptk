 ...
 *
 * $Author: sacha $
 * $Date: 2007-06-26 14:17:40 +0200 (Tue, 26 Jun 2007) $
 * $Revision: 1080 $
 *
 */
...
class MP_Convolution_FFT_c:public MP_Convolution_c {
 ...
 private:
   ...
   *
   * \f[ fftSize = 2 * anywaveTable->filterLen \f]
   */
  unsigned long int fftSize;

  /** \brief output of the \a fftPlan of length numFreqs (because using real-valued signals)*/
  MP_Real_t * signalFftBufferRe;
  MP_Real_t * signalFftBufferIm;

  /** \brief input of the \a ifftPlan of length numFreqs (because using real-valued signals) */
  MP_Real_t * outputFftBufferRe;
  MP_Real_t * outputFftBufferIm;
  /** \brief output of the \a ifftPlan of length fftSize */
  MP_Real_t *outputBuffer;

  /** \brief storage of the FFT of the filters of length numFreqs *
   * anywaveTable->numFilters * anywaveTable->numChans
   */
  MP_Real_t * filterFftStorageRe;
  MP_Real_t * filterFftStorageIm;
   ...
   * filterFftStorage[j + filterIdx*numFreqs +
   * chanIdx*numFilters]
   */
  MP_Real_t *** filterFftBufferRe;
  MP_Real_t *** filterFftBufferIm;

  /** \brief storage of the FFT of the REAL filters of length
   * numFreqs * anywaveTable->numFilters * anywaveTable->numChans
   */
  MP_Real_t * filterRealFftStorageRe;
  MP_Real_t * filterRealFftStorageIm;
   ...
   * filterRealFftStorage[j + filterIdx*numFreqs + chanIdx*numFilters]
   */
  MP_Real_t*** filterRealFftBufferRe;
  MP_Real_t*** filterRealFftBufferIm;
  /** \brief storage of the FFT of the HILBERT filters of length
   * fftHilbertSize * anywaveTable->numFilters * anywaveTable->numChans
   */
  MP_Real_t* filterHilbertFftStorageRe;
  MP_Real_t* filterHilbertFftStorageIm;
   ...
   */
  MP_Real_t*** filterHilbertFftBufferRe;
  MP_Real_t*** filterHilbertFftBufferIm;
  /** brief array of size numFilters of pointers to the output of the
   * first slice of the method circular_convolution
   *
   */
  MP_Real_t** outputRealBufferAdd;

  /** brief array of size numFilters of pointers to the output of the
   * second slice of the method circular_convolution
   *
   */
  MP_Real_t** outputRealBufferNew;

  /** brief array of size numFilters of pointers to the output of the
   * first slice of the method circular_convolution
   *
   */
  MP_Real_t** outputHilbertBufferAdd;

  /** brief array of size numFilters of pointers to the output of the
   * second slice of the method circular_convolution
   *
   */
  MP_Real_t** outputHilbertBufferNew;
  
  MP_FFT_Interface_c * fftInterface;
 ...
 private:
   ...
   *
   * initialize :
   * - \a fftSize \f[ fftSize = 2 * anywaveTable->filterLen \f]
   * - \a numFreqs \f[ numFreqs = \frac{fftSize}{2} + 1 \f]
   * 
   * allocate (using fftw_malloc)
   * - \a signalFftBuffer  (size \a numFreqs) (and copies zeros on the second half of the buffer)
   * - \a outputFftBuffer  (size \a numFreqs)
   * - \a signalBuffer     (size \a fftSize for zero-padding)
   * - \a outputBuffer     (size \a fftSize for zero-padding)
   *
   * allocate (using fftw_malloc) and compute the DFTs to fill in
   * - \a filterFftStorage (size \a numFreqs * \a anywaveTable->numFilters * \a anywaveTable->numChans)
 ...
 private: 
   ...
   */
  MP_Real_t* slice( unsigned long int sliceIdx, MP_Real_t* inputStart );
   ...
   */
  void circular_convolution( MP_Real_t* pSlice, MP_Real_t* pNextSlice, unsigned short int chanIdx, unsigned long int firstFrameSample, unsigned long int numFramesAdd, unsigned long int numFramesNew );

  void circular_convolution_hilbert( MP_Real_t* pSlice, MP_Real_t* pNextSlice, unsigned short int chanIdx, unsigned long int firstFrameSample, unsigned long int numFramesAdd, unsigned long int numFramesNew );
