...
MP_Anywave_Table_c* MP_Anywave_Table_c::copy( void ) {
  ...
  if ((numChans != 0) && (numFilters != 0) && (filterLen != 0)) {
    /* check that multiplying the three dimensions will not go over the maximum size of an unsigned long int */
    if ( ((double)MP_MAX_SIZE_T / (double) numChans / (double) numFilters / (double) filterLen) / (double)sizeof(MP_Sample_t) <= 1.0) {
      mp_error_msg( "MP_Anywave_Table_c::copy", "numChans [%lu] . numFilters [%lu] . filterLen [%lu] . sizeof(MP_Sample_t) [%lu] is greater than the max for a size_t [%lu]. Cannot use malloc for allocating space for storing the anywave filters. Exiting from copy().\n", numChans, numFilters, filterLen, sizeof(MP_Sample_t), MP_MAX_SIZE_T);
      
      return( NULL );
    }
  }
  numBytes = numChans * numFilters * filterLen * sizeof(MP_Sample_t);
  
  /* try to allocate room for storage */
  if ( ( newTable->storage = (MP_Sample_t *)malloc( numBytes ) ) == NULL ) {
