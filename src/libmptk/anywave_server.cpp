...
/*************************************/
/* DECLARATION OF A WAVEFORMS SERVER */
/* WITH GLOBAL SCOPE                 */
/*************************************/
/**
 * \brief Declaration of a unique waveforms server with global scope over the
 * whole library
 */
MP_Anywave_Server_c MP_GLOBAL_ANYWAVE_SERVER;
...
unsigned long int MP_Anywave_Server_c::add( char* filename ){
  ...
  /* Else, add the table */
    ...
    if ( pFile == NULL) {
      ...
      /* create the table */
      table = new MP_Anywave_Table_c( pFile );
      if ( table == NULL ) {
