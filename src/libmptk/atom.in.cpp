...
void MP_Atom_c::substract_add( MP_Signal_c *sigSub, MP_Signal_c *sigAdd ) {
  ...
  for ( chanIdx = 0 , atomIn = totalBuffer; chanIdx < numChans; chanIdx++ ) {
	...
	getAddWorker()->runAdd (this, sigAdd, pos, len, chanIdx, atomIn) ;
	...
	*ps = (MP_Sample_t)(sigVal);
    ...
    addSigEnergy = getAddWorker()->wait() ; 
...
MP_Atom_c::AddWorker * MP_Atom_c::getAddWorker() {
	if (!myAddWorker) {
		MP_Atom_c::myAddWorker = new MP_Atom_c::AddWorker() ;
	} 
	return myAddWorker ;
}

MP_Atom_c::AddWorker * MP_Atom_c::myAddWorker = NULL ;

#ifdef MULTITHREAD   
    // Static function to run the threads and pass the object context via the self pointer
    void* MP_Atom_c::AddWorker::run(void* a)
    {
      MP_Atom_c::AddWorker * myAddWorker = static_cast<MP_Atom_c::AddWorker *>(a);
      // Call the the thread function in Object context
      myAddWorker->runAddParallel () ;
      pthread_exit(0) ;
      return 0;
    }
 
	MP_Atom_c::AddWorker::AddWorker() : result (0.0, 0.0) {	
		pthread_t thread_id ;
		//create synchronization primitives and worker thread 
		
      task = new ParallelAdd();
	...
	void MP_Atom_c::AddWorker::runAdd(MP_Atom_c * self,
  				  				   MP_Signal_c *sigAdd, 
  				 				   unsigned long int pos, 
  				  				   unsigned long int len,
				 				   MP_Chan_t chanIdx,
    ...
    std::pair<double,double> MP_Atom_c::AddWorker::wait() {
    ...
    void MP_Atom_c::AddWorker::runAddParallel () {
