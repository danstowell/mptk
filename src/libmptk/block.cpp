...
unsigned long int MP_Block_c::build_atom_waveform_corr(MP_Atom_c *thisAtom,MP_Real_t *outBuffer )
{
	...
	for (unsigned int iIndex = 0; iIndex < filterLen; iIndex++)
        outBuffer[iIndex] = (outBuffer[iIndex]/thisAtom->amp[0])*thisAtom->corr[0]; // ATTENTION FONCTIONNE POUR LE CHANNEL 1 : pr�voir du multiChannel
	return filterLen;
}
...
unsigned long int MP_Block_c::build_atom_waveform_norm(MP_Atom_c *thisAtom,MP_Real_t *outBuffer )
{
    ...
    for (unsigned int iIndex = 0; iIndex < filterLen; iIndex++)
        outBuffer[iIndex] = outBuffer[iIndex]/thisAtom->amp[0]; // ATTENTION FONCTIONNE POUR LE CHANNEL 1 : pr�voir du multiChannel
    return filterLen;
}
