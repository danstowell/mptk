...
int MP_Gabor_Atom_Plugin_c::alloc_gabor_atom_param( const MP_Chan_t setNumChans )
{
	...
	if ( (corr_phase = (MP_Real_t*)calloc( setNumChans, sizeof(MP_Real_t)) ) == NULL )
    {
		mp_error_msg( func, "Can't allocate the correlation phase array.\n" );
		return( 1 );
    }
...
MP_Atom_Param_c* MP_Gabor_Atom_Plugin_c::get_atom_param( void )const{
...
/********************/
/* Waveform builder */
void MP_Gabor_Atom_Plugin_c::build_waveform_corr( MP_Real_t *outBuffer )
{
		 ...
		 chanIdx++  )
    {
		...
		dHalfChirp = (double)( chirp ) * MP_PI;
		dFreq      = (double)(  freq ) * MP_2PI;
		dPhase     = (double)( corr_phase[chanIdx] );
		dAmp       = (double)(   corr[chanIdx] );
