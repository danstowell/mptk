...
/************************/
/* Factory function     */
MP_Atom_c* MP_Mdct_Atom_Plugin_c::mdct_atom_create_empty(MP_Dict_c* dict)
{

	return new MP_Mdct_Atom_Plugin_c(dict);

}

/*************************/
/* File factory function */
MP_Atom_c* MP_Mdct_Atom_Plugin_c::create_fromxml( TiXmlElement *xmlobj, MP_Dict_c *dict)
{
	const char* func = "MP_Mdct_Atom_Plugin_c::create_fromxml(fid,mode)";

	MP_Mdct_Atom_Plugin_c* newAtom = NULL;

	/* Instantiate and check */
	newAtom = new MP_Mdct_Atom_Plugin_c(dict);
	if ( newAtom == NULL )
	{
...
MP_Atom_c* MP_Mdct_Atom_Plugin_c::create_frombinary( FILE *fid, MP_Dict_c *dict)
{
	const char* func = "MP_Mdct_Atom_Plugin_c::create_frombinary(fid,mode)";
...
/********************/
/* Void constructor */
MP_Mdct_Atom_Plugin_c::MP_Mdct_Atom_Plugin_c( MP_Dict_c* dict ):MP_Atom_c(dict)
{
...
int MP_Mdct_Atom_Plugin_c::info()
{
	...
	nChar += (int)mp_info_msg( "MDCT ATOM", "%s window (window opt=%g)\n", window_name(windowType), windowOption );
...
{
	if (dict->block[blockIdx]->s) // we have a block available for fast synthesis
...
{
	MP_Atom_Factory_c::register_new_atom_empty("mdct",&MP_Mdct_Atom_Plugin_c::mdct_atom_create_empty);
	MP_Atom_Factory_c::register_new_atom("mdct",&MP_Mdct_Atom_Plugin_c::create_fromxml,&MP_Mdct_Atom_Plugin_c::create_frombinary);
