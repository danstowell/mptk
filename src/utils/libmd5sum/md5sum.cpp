...
static void md5_core (unsigned char *, struct md5_ctx *);
...
bool md5_encode(unsigned char *szInputEncode, int iInputSize, unsigned char *OutputEncode)
{
	struct md5_ctx		stctMD5;

	// Transform the filename string into mp5 checksum 
    md5_init(&stctMD5);
    stctMD5.size = iInputSize;
	if(stctMD5.size > MD5_BUFFER)
	{
		stctMD5.buf = (unsigned char *)realloc(stctMD5.buf,stctMD5.size);
		if(stctMD5.buf == NULL)
			return false;
	}
	memcopy(stctMD5.buf, szInputEncode,stctMD5.size);
    md5_update(&stctMD5);
    md5_final(OutputEncode, &stctMD5);
	if(stctMD5.buf)
	{
		free(stctMD5.buf);
		stctMD5.buf = NULL;
...
static void md5_core (unsigned char *buffer, struct md5_ctx *context)
{
