...
void usage( void ) 
{
	...
	fprintf( stdout, "     Filters the atoms contained in bookIn.bin (or stdin), stores those which satisfy the indicated properties in bookYes.bin (or stdout)\n" );
	fprintf( stdout, "      and the others in bookNo.bin (If no output files are given, the atoms are just counted and their number is reported in stderr).\n" );
	fprintf( stdout, " \n" );
	fprintf( stdout, " Main arguments:\n" );
	fprintf( stdout, "     (bookIn.bin|-)      A book of input atoms, or stdin.\n" );
	fprintf( stdout, "     (bookYes.bin|-)     OPTIONAL: A file (or stdout) to store the book of atoms which satisfy the indicated properties.\n" );
	fprintf( stdout, "     bookNo.bin          OPTIONAL: A file to store a book of atoms which do not satisfy the indicated properties.\n" );
	fprintf( stdout, " \n" );
	...
	fprintf( stdout, "     The intervals can exclude the min or max value by using reverted braces, e.g. ]min:max] will exclude the min value.\n");
	...
	fprintf( stdout, "     Take all the atoms with a frequency lower than 50Hz and higher than 1000Hz among the first 100 atoms of\n" );
	fprintf( stdout, "     bookIn.bin, store them in bookYes.bin and store all the others in bookNo.bin:\n" );
	fprintf( stdout, " \n" );
	fprintf( stdout, " Note:\n" );
	fprintf( stdout, "     Only one instance of each property is allowed. If you want to elaborate more complicated domains, use a pipe.\n" );
	fprintf( stdout, " \n" );
	exit(0);
}
