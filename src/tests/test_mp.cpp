...
int main( int argc, char ** argv ) {
	...
	else
    {
		...
		{ 
			mp_error_msg( func, "Bad Number of arguments, test_mp require configFile as first argument, and optional (number of iteration and precision) as argument in unsigned long int and double.\n");
			return( -1 );
		}
	}
	...
	refdir = MPTK_Env_c::get_env()->get_config_path("reference");
	if(NULL==refdir) 
	{
