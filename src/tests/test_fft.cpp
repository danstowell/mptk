...
int main(int argc, char ** argv)
{
	...
	/* 2/ FFT computation for the "2 cosines" signal */
	mp_info_msg( func,"FFT computation for the 2_cosines signal \n");

    MP_Real_t	buffer1[256];
    MP_Real_t   magbuf1[257];
    MP_FFT_Interface_c * fft1 = MP_FFT_Interface_c::init( 256, DSP_HAMMING_WIN, 0.0, 512 );
    ...
    if (fft1 != NULL)
    {
        ...
        mp_fread( buffer1, sizeof(float), 256, fid );
        fclose(fid);
        fft1->exec_mag( buffer1, magbuf1);
        strOutFile = string(tmpdir);
        ...
        mp_fwrite( magbuf1, sizeof(MP_Real_t), 256, fid );
        fclose(fid);
    ...
    MP_Real_t	buffer2[8192];
    MP_Real_t   magbuf2[4097];
    mp_info_msg( func,"Testing something that makes the fft crash ... \n");
    MP_FFT_Interface_c * fft2 = MP_FFT_Interface_c::init( 2000, DSP_HAMMING_WIN, 0.0, 512 );
	...
	mp_fread( buffer2, sizeof(float), 8192, fid );
    fclose(fid);

    fft2 = MP_FFT_Interface_c::init( 8192, DSP_HAMMING_WIN, 0.0, 8192 );
    if (fft2 != NULL)
    {
        fft2->exec_mag( buffer2, magbuf2);
        strOutFile = string(tmpdir);
        ...
        mp_fwrite( magbuf2, sizeof(MP_Real_t), 512, fid );
        fclose(fid);
