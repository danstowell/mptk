...
%% SVN log:
%%   $Author: sacha $
%%   $Date: 2006-03-23 17:17:36 +0100 (Thu, 23 Mar 2006) $
%%   $Revision: 546 $
%%
...
% signal 2 - with mpd_demix
fprintf('\nProcessing signal 2 with mpd_demix');

numIt = 20;

bookName1 = [tempPath '/temp_book_1'];
bookName2 = [tempPath '/temp_book_2.bin'];
matrixName = [tempPath '/mixing_matrix.txt'];
residualName = [tempPath '/residual.wav'];
fid = fopen(matrixName,'wt');
fprintf(fid,'2 5\n');
fprintf(fid,'1.3 0.2 0.2 0.3 0.2\n');
fprintf(fid,'0.2 1.0 1.5 1.2 0.2\n');
fclose(fid);
quiet = '-v';
t_line = sprintf('%smpd_demix %s -R 1 -D %s -M %s -n %d %s %s %s', utilsPath, quiet, [tempPath '/dict1.xml'], matrixName,numIt, [tempPath '/sig2.wav'], bookName1,residualName);
unix(t_line);
% $$$ 
% $$$ t_line = sprintf('%smpf %s --index=[0:%lu] %s %s', utilsPath, quiet, numKeptAtoms - 1, bookName1, bookName2);
% $$$ unix(t_line);
t_line = sprintf('%smpr %s %s %s', utilsPath, quiet, [bookName1 '_00.bin'], [tempPath '/sig2_rec_00.wav']);
unix(t_line);
t_line = sprintf('%smpr %s %s %s', utilsPath, quiet, [bookName1 '_01.bin'], [tempPath '/sig2_rec_01.wav']);
unix(t_line);

sigRec2_00 = wavread([tempPath '/sig2_rec_00.wav']);
sigRec2_01 = wavread([tempPath '/sig2_rec_01.wav']);
...
subplot(413);
plot(sigRec2_00);
title(sprintf('chan 1 of Signal 2 reconstructed with the first %lu atoms',numKeptAtoms));
subplot(414);
plot(sigRec2_01);
title(sprintf('chan 2 of Signal 2 reconstructed with the first %lu atoms',numKeptAtoms));
