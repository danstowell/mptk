...
IF(PIPOL_IMAGE)
	IF(WIN32)
		IF(CMAKE_CL_64)
			...
			SET(MATLAB_MEX_LIBRARY "/cygdrive/y/i386/matlab-2010a-windows/extern/lib/win32/libmex.dll")
			SET(MATLAB_MX_LIBRARY "/cygdrive/y/i386/matlab-2010a-windows/extern/lib/win32/libmx.dll")
			SET(MATLAB_ENG_LIBRARY "/cygdrive/y/i386/matlab-2010a-windows/extern/lib/win32/libeng.dll")
			SET(MATLAB_INCLUDE_DIR "/cygdrive/y/i386/matlab-2010a-windows/extern/include/mex.h")
...
ELSE(PIPOL_IMAGE)
	IF(UNIX)
		...
		ELSE(APPLE)
			IF(CMAKE_SIZEOF_VOID_P MATCHES "4")
	  			# 32 bits OS
	  			FILE(GLOB MATLAB_PATHS "/opt/MATLAB_*" "/netshare/i386/matlab-2010a-*")
				FOREACH(COUNT ${MATLAB_PATHS})
					IF("${COUNT}" STRGREATER "${MATLAB_PATH}")
						SET(MATLAB_PATH "${COUNT}")
					ENDIF("${COUNT}" STRGREATER "${MATLAB_PATH}")
			...
			ELSE(CMAKE_SIZEOF_VOID_P MATCHES "4")
	  			# 64 bits OS
	  			FILE(GLOB MATLAB_PATHS "/opt/MATLAB_*" "/netshare/amd64/matlab-*")
				FOREACH(COUNT ${MATLAB_PATHS})
					IF("${COUNT}" STRGREATER "${MATLAB_PATH}")
						SET(MATLAB_PATH "${COUNT}")
					ENDIF("${COUNT}" STRGREATER "${MATLAB_PATH}")
	...
	ELSE(UNIX)
		IF(WIN32)
		    ...
		    FIND_PATH(MATLAB_ROOT "license.txt" ${MATLAB_PATH} NO_DEFAULT_PATH)
				...
				ELSE (MATLAB_ROOT)
					IF(CMAKE_CL_64)
						SET(MATLAB_ROOT "/cygdrive/y/amd64/matlab-2010a-windows")
					ELSE(CMAKE_CL_64)
						SET(MATLAB_ROOT "/cygdrive/y/i386/matlab-2010a-windows")
					ENDIF(CMAKE_CL_64)
